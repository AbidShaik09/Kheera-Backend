...
| Review fix validation | `git diff --check`; inspect deployment workflow diff | Deployments bind environment approval to the triggering revision | Passed; deploy jobs now fetch and check out `$GITHUB_SHA` in the persistent deployment directory. | Working tree before review-fix commit |
...
| 2026-09-13 | GitHub review thread identified that environment-approved deploy runs could fetch the latest branch tip instead of the approved commit. | Real issue. Updated both deploy workflows to fetch and check out the triggering `$GITHUB_SHA`; updated CI/CD docs accordingly. | Re-run `git diff --check`, push, reply, resolve thread, and request fresh review. |
...
- Follow-up review URLs and findings: Review thread https://github.com/AbidShaik09/Kheera-Backend/pull/77#discussion_r3999992979 fixed by binding deployments to `$GITHUB_SHA`.
